<template>
  <v-dialog width="800" scrollable persistent :value="true">
    <v-card :loading="loading" :disabled="loading">
      <v-card-title>
        <p class="headline">{{ $t("pages.create_reductions.title") }}</p>
        <v-spacer></v-spacer>
        ...
        <v-row>
          <v-col>
            <v-text-field
              ...
              :step="1"
              :placeholder="
                $t('pages.create_reductions.form.amount.placeholder')
              "
...
export default {
  components: {},
  created() {
    this.$permissions().authorizeOneOf(["reductions::add"]);
  },
  data() {
    return {
      dialog: true,
  ...
  methods: {
    ...
    async save() {
      ...
      this.loading = true;
      try {
        const response = await api.post("/reductions", {
          name: this.name,
          amount: this.amount,
