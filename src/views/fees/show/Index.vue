<template>
    ...
    <v-card :loading="loading" :disabled="loading" min-height="600">
        ...
        <div class="d-inline-block">
          <v-tabs center-active v-model="openedTab">
            ...
            <v-tab
              ...
              href="#students"
            >
              {{ $t("pages.show_fees.tabs.students") }}
            </v-tab>
            <v-tab v-if="$permissions().has(['fees::update'])" href="#edit">{{
              $t("misc.edit")
            }}</v-tab>

            <v-tab
              v-if="$permissions().has(['fees::remove'])"
              class="error--text"
              href="#remove"
              >{{ $t("misc.remove") }}</v-tab
            >
      ...
      <v-card-text class="pa-4">
        <v-tabs-items v-model="openedTab">
          ...
          <v-tab-item v-if="$permissions().has(['fees::update'])" value="edit">
            <fees-tab-edit :fees="fees" />
          </v-tab-item>
          <v-tab-item
            v-if="$permissions().has(['fees::remove'])"
            value="remove"
          >
            <fees-tab-remove :fees="fees" />
          </v-tab-item>
        </v-tabs-items>
      </v-card-text>
    </v-card>
  ...
  <script>
import api from "@/api";
import FeesTabBasics from "./tabs/FeesTabBasics.vue";
import FeesTabEdit from "./tabs/FeesTabEdit.vue";
import FeesTabRemove from "./tabs/FeesTabRemove.vue";
import StudentsList from "@/views/students/StudentsList.vue";
export default {
  components: { FeesTabBasics, FeesTabEdit, FeesTabRemove, StudentsList },
  async created() {
    this.$permissions().authorize(["fees::retrieve"]);
    await this.loadFees();
  },
  data() {
    return {
