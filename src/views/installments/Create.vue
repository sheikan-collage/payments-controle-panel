<template>
  <v-dialog width="1000" scrollable persistent :value="true">
    <v-card :loading="loading" :disabled="loading">
      <v-card-title>
        <p class="headline">{{ $t("pages.create_installments.title") }}</p>
        <v-spacer></v-spacer>
        ...
        <v-row class="my-0 py-0">
          <v-col>
            <v-text-field
              type="text"
              :placeholder="
                $t('pages.create_installments.form.name.placeholder')
              "
        ...
        <v-row class="my-0">
          <v-col class="my-0">
            <v-checkbox
              v-model="autoAdjust"
              :label="
                $t('pages.create_installments.form.divisions.auto_adjust')
              "
              :hide-details="true"
            ></v-checkbox>
          </v-col>
  ...
  methods: {
    ...
    addNewDivision() {
      ...
      if (this.autoAdjust && lastValue) {
        ...
        console.log(val);
        this.divisions[this.divisions.length - 1] = Math.ceil((lastValue + 100 - Number(this.divisionsSum)) / 2);
      }
